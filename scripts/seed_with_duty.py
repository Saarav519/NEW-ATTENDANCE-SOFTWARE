...
async def seed_database():
    ...
    # Rahul's salary and daily rate
    rahul_salary = 50000
    daily_rate = rahul_salary / 26  # ₹1,923.08 per day
    
    print(f"\n📊 Rahul's Daily Rate: ₹{daily_rate:.2f}")
    
    # SEED ALL 31 DAYS OF DECEMBER 2025 FOR RAHUL
    print("\n📅 Creating 31 days attendance with Daily Duty Amount...")
    ...
    for day in range(1, 32):
        ...
        if day in half_day_dates:
            ...
            daily_duty = round(daily_rate, 2)  # Full day = full duty
            full_days += 1
        
        total_conveyance += conveyance
        total_duty += daily_duty
        ...
        print(f"  Dec {day:02d}: {attendance_status:10} | Duty: ₹{daily_duty:>8,.2f} | Conv: ₹{conveyance}")
    
    await db.attendance.insert_many(attendance_records)
    
    print(f"\n📊 ATTENDANCE SUMMARY:")
    print(f"   Full Days: {full_days} × ₹{daily_rate:.2f} = ₹{full_days * daily_rate:,.2f}")
    print(f"   Half Days: {half_days} × ₹{daily_rate/2:.2f} = ₹{half_days * daily_rate/2:,.2f}")
    print(f"   Absent:    {absent_days} × ₹0 = ₹0")
    print(f"   ─────────────────────────────")
    print(f"   Total Duty Earned: ₹{total_duty:,.2f}")
    ...
    payslip = {
        ...
        "breakdown": {
            ...
            "total_duty_earned": round(total_duty, 2),
            "audit_expenses": 0,
            "advance_deduction": 0,
    ...
    print(f"   Special Allow (16%):   ₹{special_allowance:>10,.2f}")
    ...
    await db.cash_out.insert_one({
        "id": "CO001", "category": "salary",
        "description": "Salary - Rahul Kumar (December 2025)",
        "amount": net_pay, "date": "2026-01-05",
        "month": "January", "year": 2026, "is_auto": True,
        "created_at": datetime.utcnow().isoformat()
    })
    
    print("\n✅ DONE! Test with: ADMIN001/admin123, TL001/tl001, EMP001/emp001")
    client.close()
